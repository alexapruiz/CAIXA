...
entrada = open(sys.path[0] + "\\Arquivos\\DESC3.txt",mode='r',encoding='UTF-8')
...
for linha in entrada:
    ...
    if linha.find("VERSIONED OBJECT BASE") > -1:
        if (VOB == ""):
            ...
            if len(SERVIDOR) > 0 and ACL_ATIVADO == True and Possui_Grupos_DF2222 == True:
    ...
    if linha.find("GROUP CORPCAIXA\G DF5222 ") > -1:
        if GRUPOS_COMUNIDADE.find(linha[28:]) > 0:
            if len(COMUNIDADE) > 0:
                COMUNIDADE = COMUNIDADE + "&&" + linha[25:]
            else:
                COMUNIDADE += linha[25:]    

    if linha.find("GROUP CORPCAIXA\G DF5222 ") != -1:
        if GRUPOS_FABRICAS.find(linha[28:]) > 0:
            if len(FABRICA) > 0:    
                FABRICA = FABRICA + "&&" + linha[25:]
            else:
                FABRICA += linha[25:]

    if linha.find("GROUP CORPCAIXA\G DF5222 ") != -1 and linha.find("CC_CERTIFICACAO") == -1:
        Possui_Grupos_DF2222 = True
